...
const uniqueAssignments = [
  ...new Set(studentData.map((user) => user.assignment)),
].length;
for (let a = 0; a < uniqueAssignments; a++) {
  const assignmentAllUsers = studentData.filter(
    (item) => item.assignment === studentData[a].assignment
  );
  ...
  let averageJoy =
    joyPerAssignment.reduce((accumulator, current) => accumulator + current) /
    assignmentAllUsers.length;
  let averageDifficulty =
    difficultyPerAssignment.reduce(
...
export { studentData, studentNames, allAveragesPerAssignment };
